...
# ensure that 'BDE_fragments' module path is added to sys.path
...
def load_model_and_preprocessor():
    ...
    # # Due to previous issues with pygecko imports, using relative paths
    # preprocessor_module_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'preprocess_inputs_cfc.py'))
    # if preprocessor_module_path not in sys.path:
    #     sys.path.append(preprocessor_module_path)
    #
    # import preprocess_inputs_cfc
    ...
    # Use the correct method call with the filename
    #preprocessor = nfp.SmilesBondIndexPreprocessor.from_json(PREPROCESSOR_PATH)
...
def preprocess_smiles(smiles_list : list,  batch_size : int =1000): #preprocessor,
    import tensorflow as tf
    from preprocess_inputs_cfc import preprocessor
